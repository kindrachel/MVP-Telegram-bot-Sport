    ...
    try:
        ...
        if challenge:
            ...
            completion_text = (
                f"🎉 Отлично выполнено!\n\n"
                f"'{challenge.text}'\n\n"
                f"💎 Получено: +{challenge.points} баллов!"
                f"{level_up_msg}"
            )
        else:
    ...
    try:
        ...
        for place in leaderboard:
            ...
            leaderboard_text += (
                ...
                f"   💎 {place['points']} баллов | {get_level_name(place['level'])}\n"
...
async def process_custom_challenge_text(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        custom_challenge = Challenge(
            ...
            text=challenge_text,
            points=2,  # 0 баллов за свой челлендж
        ...
        success_text = (
            ...
            f"💎 Награда: 2 балла за активность\n\n"
            f"Теперь вы можете найти его в разделе 'Активность' → 'Активные челленджи'\n\n"
            f"💡 *Примечание:* Свой челлендж создается для личной мотивации"
        )
