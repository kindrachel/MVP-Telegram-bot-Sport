...
async def schedule_change_text_start(callback: types.CallbackQuery, state: FSMContext) -> None:
    ...
    try:
        ...
        message_text = (
            ...
            f"Поддерживается форматирование [Markdown](https://telegra.ph/Formatirovanie-Markdown-01-12)\n\n"
            f"Для отмены нажмите /cancel"
        )
        ...
        await callback.message.edit_text(message_text, parse_mode="Markdown", reply_markup=kb, disable_web_page_preview=True)
...
{difficulty_emoji} Сложность: {challenge.get('difficulty', 'medium')} | ⭐ Баллы: {challenge.get('points', 10)}
        ...
        for time_slot, challenges in challenges_by_time.items():
            if challenges:
                ...
                for title, count in list(challenge_titles.items())[:3]:  # Показываем максимум 3
                    ...
                    schedule_text += f"  ⭐ Баллы: {challenge.points}\n"
