...
from database import User, Organization, UserRole, Challenge, get_session
from keyboards import org_type_keyboard, get_main_menu_keyboard
...
async def _save_user_to_db(callback, state: FSMContext, data=None):
    ...
    try:
        ...
        if user_id in config.admin_ids:
            ...
            await callback.message.answer("Выбери действие:", reply_markup=get_main_menu_keyboard(user_id))
...
@router.callback_query(F.data == "back_to_direction")
async def back_to_direction_handler(callback: types.CallbackQuery, state: FSMContext) -> None:
    """Возврат к выбору направления"""
    data = await state.get_data()
    name = data.get('name')

    # Редактируем сообщение с выбором направления
    try:
        await callback.message.edit_media(
            media=InputMediaPhoto(
                media=registartion_pic,
...
@router.callback_query(F.data.startswith("superadmin_edit_member_"))
async def superadmin_edit_member(callback: types.CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    ...
    member_id = int(callback.data.replace("superadmin_edit_member_", ""))
    ...
    try:
        ...
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="👤 Изменить имя", callback_data=f"edit_name_{member.id}")],
            [InlineKeyboardButton(text="📱 Изменить телефон", callback_data=f"edit_phone_{member.id}")],
            [InlineKeyboardButton(text="◀️ Назад", callback_data=f"superadmin_view_member_{member.id}")]
        ])
        
        text = f"✏️ Редактирование участника: {member.name}\n\nВыберите что изменить:"
...
async def edit_member_name(callback: types.CallbackQuery, state: FSMContext):
    member_id = int(callback.data.replace("edit_name_", ""))
    await state.update_data(edit_member_id=member_id)
    
    await callback.message.edit_text("Введите новое имя:", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="❌ Отмена", callback_data=f"superadmin_view_member_{member_id}")]
    ]))
    
    await state.set_state("editing_member_name")

@router.message(StateFilter("editing_member_name"))
async def process_edit_name(message: types.Message, state: FSMContext):
    new_name = message.text.strip()
    data = await state.get_data()
    member_id = data.get("edit_member_id")
    ...
    try:
        ...
        if member:
            ...
            session.commit()
            kb = InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="◀️ Назад к профилю", callback_data=f"superadmin_view_member_{member_id}")]
            ])
            await message.answer("✅ Имя обновлено", reply_markup=kb)
        else:
...
async def edit_member_phone(callback: types.CallbackQuery, state: FSMContext):
    member_id = int(callback.data.replace("edit_phone_", ""))
    await state.update_data(edit_member_id=member_id)
    
    await callback.message.edit_text("Введите новый телефон:", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="❌ Отмена", callback_data=f"superadmin_view_member_{member_id}")]
    ]))
    
    await state.set_state("editing_member_phone")

@router.message(StateFilter("editing_member_phone"))
async def process_edit_phone(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if member:
            ...
            await message.answer("✅ Телефон обновлен", reply_markup=kb)
        else:
            await message.answer("❌ Участник не найден")
    finally:
        session.close()
...
@router.callback_query(F.data.startswith("superadmin_delete_member_"))
async def superadmin_delete_member(callback: types.CallbackQuery):
    user_id = callback.from_user.id
    ...
    member_id = int(callback.data.replace("superadmin_delete_member_", ""))
    ...
    try:
        ...
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="✅ Да, удалить", callback_data=f"confirm_delete_{member.id}")],
            [InlineKeyboardButton(text="❌ Отмена", callback_data=f"superadmin_view_member_{member.id}")]
        ])
        
        text = f"🗑️ Вы уверены, что хотите удалить участника {member.name}?\n\nЭто действие необратимо!"
...
async def confirm_delete_member(callback: types.CallbackQuery):
    member_id = int(callback.data.replace("confirm_delete_", ""))
    ...
    try:
        ...
        # Delete challenges
        session.query(Challenge).filter(Challenge.user_id == member.user_id).delete()
...
@router.callback_query(F.data.startswith("superadmin_member_activity_"))
async def superadmin_member_activity(callback: types.CallbackQuery):
    user_id = callback.from_user.id
    ...
    member_id = int(callback.data.replace("superadmin_member_activity_", ""))
    ...
    try:
        ...
        text = f"📊 История активностей: {member.name}\n\n"
        text += f"Последняя активность: {member.last_active.strftime('%d.%m.%Y %H:%M') if member.last_active else 'Неизвестно'}\n"
        text += f"Зарегистрирован: {member.registered_at.strftime('%d.%m.%Y %H:%M')}\n"
...
@router.callback_query(F.data.startswith("superadmin_member_challenges_"))
async def superadmin_member_challenges(callback: types.CallbackQuery):
    user_id = callback.from_user.id
    ...
    member_id = int(callback.data.replace("superadmin_member_challenges_", ""))
    ...
    try:
        ...
        challenges = session.query(Challenge).filter(Challenge.user_id == member.user_id).order_by(Challenge.created_at.desc()).limit(10).all()
        
        text = f"🎯 Челленджи участника: {member.name}\n\n"
        
        if not challenges:
            text += "Нет челленджей"
        else:
            for c in challenges:
                status_icon = {"ACTIVE": "🟢", "COMPLETED": "✅", "FAILED": "❌", "PENDING": "⏳"}.get(c.status, "⚪")
                text += f"{status_icon} {c.text[:30]}... | 💎 {c.points}\n"
