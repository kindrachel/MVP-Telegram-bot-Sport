...
def get_main_menu_keyboard(user_id: int):
    ...
    try:
        ...
        if user and user.role == UserRole.SUPER_ADMIN.value:
            return superadmin_main_menu_keyboard()
        elif user and user.role == UserRole.ORG_ADMIN.value:
...
def superadmin_main_menu_keyboard():
    """Главное меню для суперадминов"""
    return ReplyKeyboardMarkup(
        keyboard=[
...
def update_member_fields_keyboard():
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="💎 Добавить Баллы", callback_data="update_points")],
        [InlineKeyboardButton(text="◀️ Назад", callback_data="back_to_menu")]
    ])