    ...
    def create_daily_report_pdf(report_data: Dict) -> BytesIO:
        ...
        try:
            ...
            stats_table_data = [
                ...
                [Paragraph('Заработано баллов', normal_style), 
            ...
            if member_reports:
                ...
                for idx, member_report in enumerate(member_reports, 1):
                    ...
                    story.append(Paragraph(
                        f"<b>{idx}. {user_data.get('name', 'Неизвестно')} | Уровень: {user_data.get('level', 1)} | Баллы: {user_data.get('points', 0)}</b>",
                        heading_style
                    ))
    ...
    def create_csv_report(report_data: Dict, report_type: str = "daily") -> BytesIO:
        ...
        try:
            if report_type == "daily":
                ...
                writer.writerow(["Заработано баллов", stats.get('total_points_earned', 0)])
                
            elif report_type == "members":
                # Отчет по игрокам
                writer.writerow(["Отчет по игрокам", datetime.now().strftime('%d.%m.%Y')])
                writer.writerow([])
                writer.writerow(["Имя", "Уровень", "Баллы", "Челленджи", "Выполнено", "Процент", "Опросы", "Энергия"])
    ...
    def create_personal_report_pdf(report_data: Dict) -> BytesIO:
        ...
        try:
            ...
            stats_table_data = [
                ...
                [Paragraph('Заработано баллов', normal_style), Paragraph(str(stats.get('total_points', 0)), normal_style)],
            ...
            if progress:
                ...
                progress_table_data = [
                    ...
                    [Paragraph('Общие Баллы', normal_style), Paragraph(str(progress.get('current_points', 0)), normal_style)],
